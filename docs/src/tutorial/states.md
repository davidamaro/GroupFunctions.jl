...
Now that we have a labeling scheme for basis states, the natural question is: how do transition amplitudes between these states depend on the unitary transformation $U$? For bosons, this involves permanents; for fermions, determinants; for mixed symmetry, a generalization called the ==Grabmeier-Kerber formula==. See the [tutorial discussion of group functions](group_functions.md) or the corresponding [background page](../background/group_functions.md).
