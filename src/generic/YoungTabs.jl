...
function determine_corner(path::Array{Tuple{Int64,Int64},1})
    ...
    if violation_col == 1
        ...
        max_col = maximum(t -> t[2], path[1:end-1])
        last_pos_by_col = Vector{Tuple{Int64,Int64}}(undef, max_col)
        fill!(last_pos_by_col, (0, 0))
        @inbounds for pos in path[1:end-1]
            last_pos_by_col[pos[2]] = pos
        end

        prev_col = violation_col - 1
        while (corner_row <= violation_row && prev_col > 0)
            pos = last_pos_by_col[prev_col]
            @assert pos != (0, 0)
            corner_row, corner_col = pos
            prev_col -= 1
        end
    end
